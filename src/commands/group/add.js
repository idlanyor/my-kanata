...
export default {
    ...
    execute: async (sock, m, args, text) => {
        ...
        const findInviteMeta = (node) => {
            ...
            const inviteExpirationRaw = attrs.invite_exp || attrs.expiration || attrs.inviteExpiration;

            if (inviteCode) {
                return {
                    inviteCode,
                    inviteExpiration: Number(inviteExpirationRaw) || Math.floor(Date.now() / 1000) + 3 * 24 * 60 * 60
                };
            }
        ...
        try {
            ...
            for (const res of response) {
                ...
                if (statusCode === '403' || statusCode === '408') {
                    ...
                    try {
                        ...
                        const inviteExpiration = inviteMeta?.inviteExpiration || Math.floor(Date.now() / 1000) + 3 * 24 * 60 * 60;
            ...
            if (pendingInvite.length > 0) lines.push(`Butuh invite manual/privacy: ${pendingInvite.length}`);
