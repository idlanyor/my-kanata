...
export default {
    ...
    handleButton: async (sock, m, isOwner) => {
        ...
        if (buttonId === 'order3_yes' || textValue === '✅ Proceed') {
            ...
            try {
                await sock.sendMessage(getAdminJid(), {
                    ...
                    buttons: [
                        { buttonId: `order3_success_${orderData.invoicePreview}`, buttonText: { displayText: `Success ${orderData.invoicePreview}` }, type: 1 },
                        { buttonId: `order3_failed_${orderData.invoicePreview}`, buttonText: { displayText: `Failed ${orderData.invoicePreview}` }, type: 1 }
                    ]
                });
            } catch (err) {
        ...
        if (parsedSuccessInvoice) {
            ...
            if (index === -1 || orders[index].status !== 'PENDING') return m.reply('Order tidak ditemukan.');
            ...
            await sock.sendMessage(orderData.jid, { text: `✅ *ORDER COMPLETED*\n\n🧾 *Invoice:* ${orderData.invoicePreview}\n📦 *Product:* ${orderData.productCode}\n🎯 *ID:* ${orderData.uid}\n💳 *Paid:* ${formatCompactMyr(orderData.finalPriceMyr)}` });
        ...
        if (parsedFailedInvoice) {
            ...
            await sock.sendMessage(m.chat, {
                ...
                buttons: [
                    { buttonId: `order3_failreason_invalidid_${parsedFailedInvoice}`, buttonText: { displayText: 'Invalid ID' }, type: 1 },
                    { buttonId: `order3_failreason_unavailable_${parsedFailedInvoice}`, buttonText: { displayText: 'Unavailable' }, type: 1 }
                ]
            }, { quoted: m });
            return true;
        }
        ...
        if (parsedFailReason.reasonCode && parsedFailReason.invoice) {
            ...
            if (index === -1 || orders[index].status !== 'WAITING_FAIL_REASON') return m.reply('Sesi alasan gagal tidak ditemukan.');
            ...
            await sock.sendMessage(orderData.jid, { text: `❌ *ORDER FAILED*\n\nInvoice: ${orderData.invoicePreview}\n💸 *Refunded:* ${formatCompactMyr(orderData.finalPriceMyr)}\n*Reason:* ${failReason}` });
    ...
    execute: async (sock, m, args) => {
        ...
        if (args.length === 0) {
            const entries = Object.entries(priceList).map(([code, myrPrice]) => ({
                code,
                finalPrice: Math.round((Number(myrPrice) * (1 + markupPercentage / 100)) * 100) / 100
            })).sort((a, b) => a.finalPrice - b.finalPrice);

            const list = entries.slice(0, 20).map((it, i) => `${i + 1}. *${it.code}* — ${formatCompactMyr(it.finalPrice)}`).join('\n');
            return m.reply(`🛒 *ORDER3 PRICE LIST*\n\n${list}\n\n📌 Format: .order3 <code> <id> <server>`);
        }

        const productCode = args[0]?.toUpperCase();
        if (!priceList[productCode]) return m.reply(`Produk tidak ditemukan.`);

        const finalPriceMyr = Math.round((parseFloat(priceList[productCode]) * (1 + markupPercentage / 100)) * 100) / 100;
        ...
        orders.push({
            type: 'order3', status: 'WAITING_CONFIRM', jid, nomor, pushname: m.pushName,
            invoicePreview, productCode, finalPriceMyr, uid: args[1], sid: args[2], createdAt: Date.now()
        });
        saveOrder3Orders(orders);
