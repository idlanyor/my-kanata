    ...
    execute: async (sock, m, args, text) => {
        ...
        try {
            ...
            const { documentMessage: packUpload } = await prepareWAMessageMedia(
                { document: zipBuffer, mimetype: 'application/octet-stream', fileName: `${packId}.stickerpack` },
                { upload: sock.waUploadToServer }
            );
            ...
            await m.reply('Ultimate Native Pack sent! If this fails, WA might be blocking custom packs.');
