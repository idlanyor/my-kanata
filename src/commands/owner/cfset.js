...
export default {
    ...
    execute: async (sock, m, args, text) => {
        ...
        if (!args[0]) {
            return m.reply(`*── 「 CLOUDFLARE CONFIG 」 ──*\n\n` +
        ...
        if (action === 'zone') {
            ...
            if (subAction === 'add') {
                ...
                const zoneId = args[3];
                if (!domain || !zoneId) return m.reply('❌ Format: .cfset zone add <domain.com> <zone_id>');
            ...
            if (subAction === 'list') {
                if (botSettings.cfZones.length === 0) return m.reply('❌ Belum ada Zone ID yang terdaftar.');
