...
export default {
    ...
    execute: async (sock, m, args, text) => {
        ...
        try {
            ...
            console.log(`[DEBUG] CURL ${config.method} ${config.url} - Type: ${contentType} - Size: ${buffer.length} - Status: ${response.status}`);
            ...
            if (contentType.includes('image')) {
                ...
                await sock.sendMessage(m.chat, { video: buffer, caption, mimetype: contentType }, { quoted: m });
            } else if (contentType.includes('audio')) {
                await sock.sendMessage(m.chat, { audio: buffer, mimetype: contentType, caption }, { quoted: m });
            } else if (contentType.includes('application/json') || contentType.includes('text') || buffer.length < 50000) {
