...
class Gopay {
    ...
    async getHistory(limit = 10) {
        try {
            const { data } = await axios.get(`${this.baseUrl}/wallet/history?page=1&limit=${limit}`, {
                headers: this.headers
            });
            
            if (!data.success) throw new Error(data.errors?.[0]?.message || 'Failed to fetch history');
