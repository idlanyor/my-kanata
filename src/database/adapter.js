...
const applyFilter = (qb, key, value) => {
    ...
    if (value !== null && typeof value === 'object' && !Array.isArray(value) && !(value instanceof Date) && !Buffer.isBuffer(value)) {
        for (const op of Object.keys(value)) {
            const v = value[op];
            switch (op) {
                case '$eq': qb.where(key, v); break;
                case '$ne': qb.whereNot(key, v); break;
                case '$gt': qb.where(key, '>', v); break;
                case '$gte': qb.where(key, '>=', v); break;
                case '$lt': qb.where(key, '<', v); break;
                case '$lte': qb.where(key, '<=', v); break;
                case '$in': qb.whereIn(key, Array.isArray(v) ? v : []); break;
                case '$nin': qb.whereNotIn(key, Array.isArray(v) ? v : []); break;
                case '$exists': v ? qb.whereNotNull(key) : qb.whereNull(key); break;
                case '$like': qb.where(key, 'like', v); break;
                case '$ilike': qb.where(key, 'like', v); break; // SQLite LIKE is case-insensitive for ASCII by default
                default: qb.where(key, v); break;
            }
        }
        return;
    }
...
const decomposeUpdate = (update) => {
    ...
    if (!update || typeof update !== 'object') return { set, increments, jsonExtracts, push, pull, addToSet, unsets, muls, renames };
    ...
    for (const op of Object.keys(update)) {
        ...
        } else if (op === '$pushAll') { // alias many dialects use
...
const applyUpdate = (qb, decomp) => {
    ...
    for (const p of decomp.push) {
        ...
        if (p.each) {
            ...
            qb.update(p.column, qb.client.raw(
    ...
    for (const a of decomp.addToSet) {
        ...
        qb.update(a.column, qb.client.raw(
            ...
            [col, JSON.stringify(a.value), col, col, col, JSON.stringify(a.value), col, col, JSON.stringify(a.value)]
        ));
    }
...
const parseSort = (sortSpec) => {
    ...
    if (typeof sortSpec === 'string') {
        const out = {};
        for (const part of sortSpec.split(',').map(s => s.trim()).filter(Boolean)) {
...
const buildQuery = ({ knex, tableName, filter = {}, schema, isOne = false }) => {
    ...
    const query = {
        ...
        async exec() {
            ...
            const rows = await qb;
            if (state.lean) {
                // Return rows directly (knex's rows are plain objects, no doc methods)
                return state.isOne ? (rows[0] || null) : rows;
            }
            const wrapped = rows.map((r) => wrapDocument(r, { knex, tableName, isNew: false, schema }));
...
export const defineModel = (tableName, schema = { paths: {}, options: {} }, opts = {}) => {
    ...
    const Model = {
        ...
        async findOneAndUpdate(filter = {}, update = {}, options = {}) {
            ...
            if (options.upsert) {
                ...
                if (!existing) {
                    ...
                    if (useTimestamps) {
                        insertDoc.created_at = knex.fn.now();
                        insertDoc.updated_at = knex.fn.now();
                    }
            ...
            if (options.new) {
                ...
                return wrapDocument(fresh, { knex, tableName, isNew: false });
            }
            return null;
        },
        ...
        async findOneAndDelete(filter = {}) {
            ...
            if (!row) return null;
            await qb.delete();
            return wrapDocument(row, { knex, tableName, isNew: false });
        },
        ...
        async create(docs) {
            ...
            const wrapped = rows.map(r => wrapDocument(r, { knex, tableName, isNew: false }));
        ...
        async aggregate(pipeline = []) {
                        ...
                        if (!rows) {
                            ...
                            rows = mat.filter((r) => {
                                for (const fk of Object.keys(val)) {
                                    const fv = val[fk];
                                    if (fv && typeof fv === 'object' && !Array.isArray(fv) && !Buffer.isBuffer(fv)) {
                                        for (const op of Object.keys(fv)) {
                                            const v = fv[op];
                                            const cv = r[fk];
                                            switch (op) {
                                                case '$eq': if (cv !== v) return false; break;
                                                case '$ne': if (cv === v) return false; break;
                                                case '$gt': if (!(cv > v)) return false; break;
                                                case '$gte': if (!(cv >= v)) return false; break;
                                                case '$lt': if (!(cv < v)) return false; break;
                                                case '$lte': if (!(cv <= v)) return false; break;
                                                case '$in': if (!v.includes(cv)) return false; break;
                                                case '$nin': if (v.includes(cv)) return false; break;
                                            }
                                        }
                                    } else if (r[fk] !== fv) {
                                        return false;
            ...
            for (const { key, val } of postStages) {
                if (key === '$group') {
                    ...
                    const cleanGroupKey = typeof _id === 'string' && _id.startsWith('$') ? _id.slice(1) : _id;
                    const groupKeyFn = cleanGroupKey === null || cleanGroupKey === undefined ? null : r => r[cleanGroupKey];
                        ...
                        if (!buckets.has(k)) {
                            ...
                            for (const f of Object.keys(fields)) {
                                ...
                                if (fSpec && typeof fSpec === 'object') {
                                    if (fSpec.$sum !== undefined) {
                                        const fieldName = typeof fSpec.$sum === 'string' && fSpec.$sum.startsWith('$') ? fSpec.$sum.slice(1) : null;
                                        if (fieldName) bucket[f] = Number(r[fieldName]) || 0;
                                        else bucket[f] = Number(fSpec.$sum) || 0;
                                    }
                                    else if (fSpec.$avg !== undefined) bucket[f] = { __avg: 0, __count: 0 };
                                    else if (fSpec.$min !== undefined) bucket[f] = r[String(fSpec.$min).replace('$', '')];
                                    else if (fSpec.$max !== undefined) bucket[f] = r[String(fSpec.$max).replace('$', '')];
                                    else if (fSpec.$first !== undefined) bucket[f] = r[String(fSpec.$first).replace('$', '')];
                                    else if (fSpec.$last !== undefined) bucket[f] = r[String(fSpec.$last).replace('$', '')];
                                    else if (fSpec.$push !== undefined) bucket[f] = [r[String(fSpec.$push).replace('$', '')]];
                            ...
                            for (const f of Object.keys(fields)) {
                                ...
                                if (fSpec && typeof fSpec === 'object') {
                                    if (fSpec.$sum !== undefined) {
                                        const fieldName = typeof fSpec.$sum === 'string' && fSpec.$sum.startsWith('$') ? fSpec.$sum.slice(1) : null;
                                        if (fieldName) bucket[f] = (bucket[f] || 0) + (Number(r[fieldName]) || 0);
                                        else bucket[f] = (bucket[f] || 0) + (Number(fSpec.$sum) || 0);
                    ...
                    materialized = Array.from(buckets.values()).map(b => {
                        if (b.__avg !== undefined) b = { ...b, _avgVal: b.__avg / (b.__count || 1) };
