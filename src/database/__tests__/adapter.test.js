...
test('countDocuments() returns the count matching the filter', async () => {
    const { User, knex } = await makeUserModel();
    await User.create([{ name: 'A', age: 1 }, { name: 'B', age: 2 }, { name: 'C', age: 1 }]);
...
test('sort/limit/skip chaining produces the expected slice', async () => {
    const { User, knex } = await makeUserModel();
    await User.create([{ name: 'A', age: 1 }, { name: 'B', age: 2 }, { name: 'C', age: 3 }, { name: 'D', age: 4 }]);
...
test('$or filter is translated to a SQL OR', async () => {
    const { User, knex } = await makeUserModel();
    await User.create([{ name: 'a', age: 1 }, { name: 'b', age: 2 }, { name: 'c', age: 3 }]);
...
test('Buffer payloads round-trip through create/find', async () => {
    ...
    const created = await Poll.create({ pollId: 'p1', chat: 'c1', question: 'Q', messageSecret: secret });
...
test('on-disk SQLite file works via tmpdir', async () => {
    const tmp = path.join(os.tmpdir(), `adapter-test-${Date.now()}-${Math.random().toString(36).slice(2)}.sqlite`);
