export const up = async (knex) => {
    await knex.schema.createTable('settings', (table) => {
        ...
        table.string('privateAiPersona').notNullable().defaultTo('Kamu adalah KanataBot, asisten pribadi AI yang cerdas.');
        table.boolean('mustJoinGroup').notNullable().defaultTo(false);
        table.boolean('smartMode').notNullable().defaultTo(false);
        table.string('groupInviteLink').notNullable().defaultTo('https://chat.whatsapp.com/I5JCuQnIo4f79JsZAGCvDD');
