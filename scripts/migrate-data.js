    ...
    for (const s of servers) {
        try {
            await knex('servers').insert({
                ...
                lastAutoRenewAt: s.lastAutoRenewAt ? new Date(s.lastAutoRenewAt).toISOString() : null,
    ...
    for (const s of settingsList) {
        try {
            await knex('settings').insert({
                ...
                disabledCommands: Array.isArray(s.disabledCommands) ? JSON.stringify(s.disabledCommands) : '[]',
                mode: s.mode || 'public',
                autoStatusRead: s.autoStatusRead ? 1 : 0,
                autoAiPrivate: s.autoAiPrivate ? 1 : 0,
                privateAiPersona: s.privateAiPersona || 'Kamu adalah KanataBot, asisten pribadi AI yang cerdas.',
                mustJoinGroup: s.mustJoinGroup ? 1 : 0,
                smartMode: s.smartMode ? 1 : 0,
                groupInviteLink: s.groupInviteLink || 'https://chat.whatsapp.com/I5JCuQnIo4f79JsZAGCvDD',
                cfToken: s.cfToken || '',
