...
async function igDl(url) {
    try {
        ...
        $('.download-items').each((i, el) => {
            ...
            const videoUrl = $(el).find('.download-items__btn a[title="Download Video"]').attr('href');
            const imageUrl = $(el).find('.download-items__btn a[title="Download Thumbnail"]').attr('href');
...
function decodeClipDown(data) {
    ...
    return vm.runInNewContext(script, { 
        Math, String, decodeURIComponent, RegExp
    });
}
