...
export const generateWAMessageContent = async (
	...
	let m: WAMessageContent = {}
        if (hasNonNullishProperty(message, 'interactive')) {
                m.interactiveMessage = WAProto.Message.InteractiveMessage.fromObject(message.interactive)
                
                // --- KANATA-BAILEYS CUSTOM: AUTO INJECT NODES ---
                if (!m.interactiveMessage.contextInfo) m.interactiveMessage.contextInfo = {}
                ;(m.interactiveMessage.contextInfo as any)._kanata_interactive = true
        } else if (hasNonNullishProperty(message, 'interactiveButtons')) {
                // ITSUKICHANN-STYLE BUTTONS HANDLING
                ...
                if (hasNonNullishProperty(message, 'title')) {
                        interactiveMessage.header = {
                                ...
                                hasMediaAttachment: msgAny.hasMediaAttachment || false
                        }
                }
                ...
                // Auto inject nodes for this style too
                if (!m.interactiveMessage.contextInfo) m.interactiveMessage.contextInfo = {}
                ;(m.interactiveMessage.contextInfo as any)._kanata_interactive = true
        } else if (hasNonNullishProperty(message, 'customPayment')) {
                const pay = (message as any).customPayment
                const interactiveMessage: proto.Message.IInteractiveMessage = {
                        header: { title: '💳 BILLING INVOICE', hasMediaAttachment: false },
                        body: { text: `Payment Request: *${pay.currency} ${pay.amount}*\n\nNote: ${pay.note}` },
                        footer: { text: `ID: ${pay.externalId}` },
                        nativeFlowMessage: {
                                buttons: [
                                        {
                                                name: 'quick_reply',
                                                buttonParamsJson: JSON.stringify({
                                                        display_text: 'LIHAT INSTRUKSI BAYAR',
                                                        id: `.payinfo ${pay.externalId}`
                                                })
                                        }
                                ]
                        }
                ...
                // Auto inject nodes flag
                if (!m.interactiveMessage.contextInfo) m.interactiveMessage.contextInfo = {}
                ;(m.interactiveMessage.contextInfo as any)._kanata_interactive = true
        } else if (hasNonNullishProperty(message, 'cards')) {
                // SANKA-BAILEYS STYLE CAROUSEL
                const msgAny = message as any
                const slides = await Promise.all(msgAny.cards.map(async (slide: any) => {
                        const { image, video, title, body, footer, buttons } = slide
                        let header: any = {}
                        if (image) {
                                const media = await prepareWAMessageMedia({ image }, options)
                                header = { ...media, hasMediaAttachment: true, title }
                        } else if (video) {
                                const media = await prepareWAMessageMedia({ video }, options)
                                header = { ...media, hasMediaAttachment: true, title }
                        } else {
                ...
                if (msgAny.text) interactiveMessage.body = { text: msgAny.text }
                if (msgAny.footer) interactiveMessage.footer = { text: msgAny.footer }
                if (msgAny.title) interactiveMessage.header = { title: msgAny.title, hasMediaAttachment: false }
                ...
                m.listMessage = WAProto.Message.ListMessage.fromObject(message.list)
                
                // --- KANATA-BAILEYS CUSTOM: AUTO INJECT NODES ---
                if (!m.listMessage.contextInfo) m.listMessage.contextInfo = {}
                ;(m.listMessage.contextInfo as any)._kanata_list = true
		...
		switch (message.type) {
	        ...
	        } else if (hasNonNullishProperty(message, 'product')) {		const { imageMessage } = await prepareWAMessageMedia({ image: message.product.productImage }, options)
		m.productMessage = WAProto.Message.ProductMessage.create({
			...
			product: {
                ...
                if (!m.listMessage.contextInfo) m.listMessage.contextInfo = {}
                ;(m.listMessage.contextInfo as any)._kanata_list = true
        } else if (hasNonNullishProperty(message, 'event')) {		m.eventMessage = {}
		...
		m.eventMessage.extraGuestsAllowed = message.event.extraGuestsAllowed
                ...
                const groupStatus = (message as any).groupStatus
                let innerContent: WAMessageContent
                if (typeof groupStatus === 'string') {
                        innerContent = { conversation: groupStatus }
                } else {
                        // Jika media, proses dulu lewat generateWAMessageContent (rekursif)
                        const mediaMsg = await generateWAMessageContent(groupStatus, options)
                        innerContent = mediaMsg
                }
                m.groupStatusMessage = { message: innerContent }
                m.groupStatusMessageV2 = { message: innerContent }
        } else if (hasNonNullishProperty(message, 'associatedChild')) {
                const child = (message as any).associatedChild
                const innerContent = typeof child === 'string' ? { conversation: child } : await generateWAMessageContent(child, options)
                m.associatedChildMessage = { message: innerContent }
        } else if (hasNonNullishProperty(message, 'statusAddYours')) {
                const addYours = (message as any).statusAddYours
                const innerContent = typeof addYours === 'string' ? { conversation: addYours } : await generateWAMessageContent(addYours, options)
                m.statusAddYours = { message: innerContent }
        } else if (hasNonNullishProperty(message, 'comment')) {
                const { text, quoted } = (message as any).comment
                m.associatedChildMessage = {
                        message: {
                                encCommentMessage: {
                                        targetMessageKey: quoted.key,
                                        encPayload: Buffer.from(text), // Dummy payload for testing
                                        encIv: randomBytes(12)
                                }
                        }
                }
        } else if (hasNonNullishProperty(message, 'newsletterAdminInvite')) {
                const invite = (message as any).newsletterAdminInvite
                m.newsletterAdminInviteMessage = {
                        newsletterJid: invite.newsletterJid,
                        newsletterName: invite.newsletterName,
                        caption: invite.caption,
                        inviteExpiration: invite.inviteExpiration instanceof Date ? Math.floor(invite.inviteExpiration.getTime() / 1000) : invite.inviteExpiration
                }
        } else if (hasNonNullishProperty(message, 'customPayment')) {
                const pay = (message as any).customPayment
                const interactiveMessage: proto.Message.IInteractiveMessage = {
                ...
                if (!m.interactiveMessage.contextInfo) m.interactiveMessage.contextInfo = {}
                ;(m.interactiveMessage.contextInfo as any)._kanata_interactive = true
        } else if (hasNonNullishProperty(message, 'lottieSticker')) {
                const sticker = (message as any).lottieSticker
                const innerContent = typeof sticker === 'string' ? { conversation: sticker } : await generateWAMessageContent(sticker, options)
                m.lottieStickerMessage = { message: innerContent }
        } else if (hasNonNullishProperty(message, 'invoice')) {
                m.invoiceMessage = WAProto.Message.InvoiceMessage.fromObject(message.invoice)
        } else if (hasNonNullishProperty(message, 'callLog')) {
                ...
                m.paymentInviteMessage = WAProto.Message.PaymentInviteMessage.fromObject(message.paymentInvite)
                        ...
                        const invoke = (message as any).botInvoke
                        const innerContent = typeof invoke === 'string' ? { conversation: invoke } : await generateWAMessageContent(invoke, options)
                        m.botInvokeMessage = { message: innerContent }
                } else if (hasNonNullishProperty(message, 'poll')) {
                        const pollCreationMessage = {
                                name: message.poll.name,
                                selectableCount: message.poll.selectableCount || 0,
                        ...
                        hasNonNullishProperty(message, 'image') ||
                        hasNonNullishProperty(message, 'video') ||
                        hasNonNullishProperty(message, 'audio') ||
                        hasNonNullishProperty(message, 'sticker') ||
                        hasNonNullishProperty(message, 'document')
                ) {
	...
	if (hasOptionalProperty(message, 'edit')) {
		m = {
	        ...
	        if (hasOptionalProperty(message, 'contextInfo') && !!message.contextInfo) {
	                let messageType = Object.keys(m)[0]! as Extract<keyof proto.IMessage, MessageWithContextInfo>
	                let key = m[messageType] as any
	
	                                // KANATA-BAILEYS FIX: Handle viewOnce wrapper
	                                if ((messageType as string) === 'viewOnceMessage' && key?.message) {
	                                        const innerType = Object.keys(key.message)[0] as any
	                                        key = key.message[innerType]
	                                }	
	                                        if (key && typeof key === 'object') {
	                                                ...
	                                                // KANATA-BAILEYS CUSTOM: Inject Newsletter Forward Metadata
	                                                if ((message as any).newsletterForward) {
	                                                        const news = (message as any).newsletterForward
	                                                        key.contextInfo = {
	                                                                ...key.contextInfo,
	                                                                forwardedNewsletterMessageInfo: {
	                                                                        newsletterJid: news.jid,
	                                                                        newsletterName: news.name,
	                                                                        serverMessageId: news.serverId || 1
	                                                                }
	                                                        }
	                                                }
	                                        }
...
export const normalizeMessageContent = (content: WAMessageContent | null | undefined): WAMessageContent | undefined => {
	...
	return content!
	        ...
	        function getFutureProofMessage(message: typeof content): proto.Message.IFutureProofMessage | null | undefined {
