...
import {
	...
	encodeSignedDeviceIdentity,
	        ...
	        import {
	                areJidsSameUser,	type BinaryNode,
...
export const makeMessagesSocket = (config: SocketConfig) => {
			...
			if (tcTokenBuffer) {
				;(stanza.content as BinaryNode[]).push({
					...
					content: tcTokenBuffer
                        ...
                        // --- KANATA-BAILEYS x ITSUKI LOGIC ---
                        const normalizedContent = normalizeMessageContent(message)
                        const isInteractive = !!(message?.interactiveMessage || normalizedContent?.interactiveMessage)
                        const isList = !!(message?.listMessage || normalizedContent?.listMessage)
                        const isButtons = !!(message?.buttonsMessage || normalizedContent?.buttonsMessage)
                        
                        // Button Args Logic (Ported from Itsuki)
                        const getButtonArgs = (msg: any): BinaryNode | undefined => {
                        ...
                        if (isInteractive || isList || isButtons) {
                                console.log(`[CORE] Injecting Itsuki-Style Node...`);
                                const node = getButtonArgs(normalizedContent || message)
                                if (node) {
                                    ;(stanza.content as BinaryNode[]).push(node as BinaryNode)
                                }
                        }
	...
	return {
		...
		getUSyncDevices,
		                messageRetryManager,
		                updateMemberLabel,
		                                sendFakeCall: async (jid: string, isVideo = false) => {
		                                        const node: BinaryNode = {
		                                                tag: 'message',
		                                                attrs: {
		                                                        to: jid,
		                                        ...
		                                        await sendNode(node)
		                                },		                updateMediaMessage: async (message: WAMessage) => {			const content = assertMediaContent(message.message)
